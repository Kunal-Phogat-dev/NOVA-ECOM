...
const menuVariants = {
  ...
  visible: { 
    opacity: 1, 
    x: 0,
    transition: { type: "tween", duration: 0.3, staggerChildren: 0.1, delayChildren: 0.1 }
  },
  exit: { 
    opacity: 0, 
    x: "-100%",
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          <div className="flex items-center">
            <button 
              className="mr-4 sm:hidden text-foreground/80 hover:text-accent transition-colors p-2 -ml-2"
          ...
          <div className="flex items-center space-x-4 sm:space-x-6">
            ...
            <Link href={isMounted && isAuthenticated ? "/account" : "/auth"} className="hidden sm:block text-foreground/80 hover:text-accent transition-colors p-2" aria-label="Account">
              <User className="h-5 w-5" />
            </Link>
            <Link href="/wishlist" className="hidden sm:block text-foreground/80 hover:text-accent transition-colors relative p-2" aria-label="Wishlist">
            ...
            <button 
              className="text-foreground/80 hover:text-accent transition-colors relative p-2"
              onClick={openCart}
              aria-label="Cart"
            >
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            <button 
              className="absolute top-6 right-6 text-muted-foreground hover:text-white p-2"
              onClick={() => setIsMobileMenuOpen(false)}
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
            
            <nav className="flex flex-col space-y-6 text-2xl font-bold tracking-widest uppercase mt-8">
              {[
                ...
                <motion.div variants={itemVariants} key={link.name}>
                  <Link href={link.path} onClick={() => setIsMobileMenuOpen(false)} className="hover:text-accent transition-colors block py-2">{link.name}</Link>
                </motion.div>
              ))}
            </nav>

            <motion.div variants={itemVariants} className="mt-auto pb-12 flex flex-col space-y-4">
              <Link href={isMounted && isAuthenticated ? "/account" : "/auth"} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center space-x-4 text-muted-foreground hover:text-accent transition-colors py-2">
